...
export const Rxjs = () => {
    ...
    const observableFromPromise = fromFetch(`${GITHUB_API}/natar10`).pipe(
        switchMap(r => r.json()),
        map(setAnswer)
    )
    const subscribeObservable = () => observableFromPromise.subscribe()
    ...
    const reposObservable = fromFetch(`${GITHUB_API}/stackbuilders/repos`).pipe(
        switchMap(r => r.json())
    )
